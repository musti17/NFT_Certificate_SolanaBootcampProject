...
mod issue_certificate {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("nft_certificate", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let course_name: String = Default::default();
		let student_name: String = Default::default();
		let certificate_image_url: String = Default::default();
		let course_description: String = Default::default();
		...
		let data = NftCertificateInstruction::IssueCertificate(crate::generated::instructions::IssueCertificateArgs{
			course_name,
			student_name,
