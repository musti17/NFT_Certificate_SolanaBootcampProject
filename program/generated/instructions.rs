...
pub enum NftCertificateInstruction {
/// Accounts:
...
impl NftCertificateInstruction {
    pub fn unpack(input: &[u8]) -> Result<Self, ProgramError> {
        let (&variant, rest) = input.split_first().ok_or(NftCertificateError::InvalidInstruction)?;

        Ok(match variant {
			0 => Self::IssueCertificate(IssueCertificateArgs::try_from_slice(rest).unwrap()),
			1 => Self::ClaimCertificate,
			2 => Self::TransferCertificate,
			3 => Self::BurnCertificate,
			_ => return Err(NftCertificateError::InvalidInstruction.into())
        })
    }
}